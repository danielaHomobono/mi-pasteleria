 ...
 * Clickeable para ir a detalles del producto
 */
export function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${product.id}`} className="block">
      <Card className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:scale-105 cursor-pointer h-full">
        {/* Imagen del producto */}
        {hasImage && (
        ...
        <CardHeader className={`pb-3 ${!hasImage ? "pt-4" : ""}`}>
          <div className="space-y-2">
            <CardTitle className="line-clamp-2 text-base md:text-lg group-hover:text-primary transition-colors">
              {product.name}
            </CardTitle>
            {hasDescription && (
